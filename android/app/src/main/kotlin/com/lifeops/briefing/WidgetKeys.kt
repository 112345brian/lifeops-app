 ...
 * here so the UI, the FCM service, the next-tasks worker, and the settings
 * screen agree without needing to read each other's code. */
object WidgetKeys {
    // Glance per-widget state (PreferencesGlanceStateDefinition).
    // Written by BriefingSyncWorker after an FCM push or setup pull, read by
    ...
    const val KEY_TOKEN = "token" // WEB_TOKEN value, appended as ?token= on pull calls
}
